...
class Solution {
    ...
    /// This implementation beats ~48% of Swift submissions
    func topKFrequent(_ nums: [Int], _ k: Int) -> [Int] {
        ...
        for num in nums {
            if let value = counter[num] { //increment count
                counter[num] = value + 1
            } else {
                counter[num] = 1
            }
        ...
        print(counter)
        var pQueue = Heap<(Int, Int)>.init { first, second in
             return first.1 > second.1
        }
        for element in counter {
            pQueue.insert((element.key, element.value))
        }
        var result = [Int]()
        for _ in 0..<k {
            if let tuple = pQueue.remove() {
...
func testTopKFrequent() {
    print(Solution.init().topKFrequent([-1, -1],1))
}
testTopKFrequent()
///Refer: https://forums.swift.org/t/heaps-structure-not-storage-and-priority-queues/31135/4
